...
function Header({ view, setView, disabled, user, onGoToBridge, onLogout, remainingTime }: HeaderProps) {
  ...
  const switcher = (
    <div className="flex items-center gap-4">
      <div className="flex justify-center p-1 space-x-2 bg-black/30 backdrop-blur-sm rounded-lg mx-auto">
        <button
          onClick={() => setView('vxdl')}
          disabled={disabled}
          className={`${baseButtonClasses} ${view === 'vxdl' ? activeButtonClasses : inactiveButtonClasses}`}
          aria-pressed={view === 'vxdl'}
          title={t('header_vxdl_tooltip')}
        >
          VXDLabs
        </button>
        <button
          onClick={() => setView('vox')}
          disabled={disabled}
          className={`${baseButtonClasses} ${view === 'vox' ? activeButtonClasses : inactiveButtonClasses}`}
        ...
        <button
          onClick={() => setView('vxog')}
          disabled={disabled}
          className={`${baseButtonClasses} ${view === 'vxog' ? activeButtonClasses : inactiveButtonClasses}`}
          aria-pressed={view === 'vxog'}
          title={t('header_vxog_tooltip')}
        >
          VXOG
        </button>
        <button
          onClick={() => setView('cos')}
          disabled={disabled}
          className={`${baseButtonClasses} ${view === 'cos' ? activeButtonClasses : inactiveButtonClasses}`}
          aria-pressed={view === 'cos'}
          title={t('header_cos_tooltip')}
        >
          COS
        </button>
         <button
          ...
          disabled={disabled}
          className={`${baseButtonClasses} ${view === 'vxpl' ? activeButtonClasses : inactiveButtonClasses}`}
          aria-pressed={view === 'vxpl'}
          title={t('header_vxpl_tooltip')}
        >
          VXPL
        </button>
      </div>
  ...
  // Unified header for VOX, VXPL, COS, VXOG
  return (
